...
int main(){
    ...
    while (cnt<=n)
    {
        for (int j=1;j<=(cnt-1)/2;j++)//i个数，最多有（i-1)/个峰数
            if(j<=(cnt-2)/2)
                dp[2][j]=min(dp[1][j],dp[0][j-1]+opr[cnt-1]);//注意他（i-1）可能达不到j（达不到j的就是0）,操作i-1和不操作i-1
            else
                dp[2][j]=dp[0][j-1]+opr[cnt-1];
        memcpy(dp[0],dp[1],(1+(cnt-2)/2)*8);//先除再乘
        memcpy(dp[1],dp[2],(1+(cnt-1)/2)*8);
        cnt++;
    }
