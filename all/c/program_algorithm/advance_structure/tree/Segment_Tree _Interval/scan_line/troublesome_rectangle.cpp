//洛谷 p5490
// 如果l+1==r为子节点注意划分区间一定要[l,mid),[mid,m)因为是开区间
//我这样的话也AC了，所以是要注意区间划分的边界问题
//线段树最初版本以点为两端，且均可取到，点不重复，若用点则包含点，若用块则会漏块(最初版本是点和)
//所以要用块的话最好以[l,r]为一个区间块所以mid要两端都能取到，见example2(这里是块和)
#include <iostream>
#include <algorithm>
#include <vector>
using namespace std;

const int maxn = 210; // 因为n是1e5，每个矩形两个边，所以最多2e5个事件
struct TrNode
{
...
void build(int node, int l, int r)
{
    ...
    if (l == r)
    { // 叶子节点表示区间[y_values[l], y_values[r])
...
void update(int node, int l, int r, int L, int R, int val)
{
    ...
    { // 完全覆盖当前区间
        tr[node].tag += val;
        if (tr[node].tag)
            tr[node].cover = tr[node].length;
        else
            tr[node].cover = (l == r) ? 0 : tr[node << 1].cover + tr[node << 1 | 1].cover;
        return;
    }
    int mid = (l + r) >> 1;
    ...
    if (tr[node].tag)
        ...
        tr[node].cover = tr[node << 1].cover + tr[node << 1 | 1].cover;
}
...
int main()
{
    ...
    for (int i = 0; i < cnt; ++i)
    {
        ...
        if (l < r)
        {
            update(1, 0, m - 2, l, r-1, lines[i].type);
        }
    }
    cout << ans << endl;
    return 0;
...
void build(int node, int l, int r) {
    tr[node].cover = tr[node].tag = 0;
    if (l + 1 == r) { // 叶子节点表示区间[y_values[l], y_values[r])
        tr[node].length = y_values[r] - y_values[l];
        return;
    }
    int mid = (l + r) >> 1;
    build(node << 1, l, mid);
...
void update(int node, int l, int r, int L, int R, int val) {
    ...
    if (L <= l && r <= R) { // 完全覆盖当前区间
        ...
        else tr[node].cover = (l + 1 == r) ? 0 : tr[node << 1].cover + tr[node << 1 | 1].cover;
        return;
    }
    int mid = (l + r) >> 1;
    update(node << 1, l, mid, L, R, val);
    update(node << 1 | 1, mid, r, L, R, val);
    if (tr[node].tag) tr[node].cover = tr[node].length;
